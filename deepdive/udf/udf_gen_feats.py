...
from ddlib.dd import dep_path_between_words, materialize_span, Span, unpack_words
...
def get_generic_features_mention(sentence, span, length_bin_size=5):
    ...
    for (i, j) in _get_substring_indices(len(sentence), MAX_KW_LENGTH):
        ...
        for dict_id in dictionaries:
            if " ".join(map(lambda x: x.word, sentence[i:j])) in \
...
def get_generic_features_relation(sentence, span1, span2, length_bin_size=5):
    ...
    for (i, j) in _get_substring_indices(len(sentence), MAX_KW_LENGTH):
        ...
        if is_in_dictionary:
            ...
            labels1 = []
            for edge in path1:
                lemmas1.append(edge.word2.word)
            ...
            labels2 = []
            for edge in path2:
                lemmas2.append(edge.word2.word)
        ...
        for j in range(1, window + 1):
            if i+j <= span.begin_word_id + span.length:
                yield "NGRAM_" + str(j) + "_[" + " ".join(
                    map(lambda x: x.word, sentence[i:i+j])) + "]"
...
def _get_min_dep_path_features(sentence, span1, span2, prefix="BETW_"):
    ...
    if min_path:
        ...
        for edge in min_path:
            min_path_lemmas.append(edge.word2.word)
...
def _get_window_features(
    ...
    try:
        for i in range(1, window + 1):
            lemma = sentence[span.begin_word_id - i].word
            try:
                float(lemma)
                lemma = "_NUMBER"
            except ValueError:
    ...
    try:
        for i in range(1, window + 1):
            lemma = sentence[span_end_idx + i].word
            try:
                float(lemma)
                lemma = "_NUMBER"
            except ValueError:
...
the dictionaries
    ...
    for i in range(window + 1):
        for j in range(span.length - i):
            phrase = " ".join(map(lambda x: x.word, sentence[j:j+i+1]))
            for dict_id in dictionaries:
...
def get_sentence(
        begin_char_offsets, end_char_offsets, words, poses,
